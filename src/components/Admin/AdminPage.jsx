...
const AdminPage = ({ user, onLogout }) => {
  ...
  const SCRIPT_URL =
    "https://script.google.com/macros/s/AKfycbzqg0NUfUh-_TgTpG-ITdRQkmR8JcJa59OG2YV6ZhOIx0tH4MvudvFjUb6M5gYCmCVL/exec";
    ...
    async (isBackground = false) => {
      try {
        ...
        const response = await fetch(
          `${SCRIPT_URL}?_t=${new Date().getTime()}`,
        );
        const data = await response.json();

        const resUser = await fetch(
          `${SCRIPT_URL}?action=getUsers&_t=${new Date().getTime()}`,
        );
        const dataUsers = await resUser.json();

        if (Array.isArray(data)) {
          setRawData(data);
          const totalTerdaftar = Array.isArray(dataUsers)
            ? dataUsers.length
            : 0;
        ...
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-6 lg:px-8">
          <div className="flex items-center gap-4 text-left">
            <button
              ...
              className="p-3 lg:hidden bg-blue-50 text-blue-600 rounded-2xl text-left"
            >
              <Menu size={24} />
            </button>
            <h2 className="text-xl font-black text-gray-800 tracking-tight italic uppercase text-left">
              {activeMenu}
            </h2>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden md:block text-right">
              <p className="text-[10px] font-bold text-gray-400 uppercase leading-none text-right">
                Administrator
              </p>
              <p className="text-sm font-black text-gray-800 text-right">
                {user?.nama || "Admin"}
              </p>
            </div>
